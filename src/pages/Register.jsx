...
export default function Register() {
        ...
        <div className="w-full max-w-md p-8 space-y-6 bg-white rounded shadow">
            ...
            <form onSubmit={handleLogin} className="space-y-4 grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" id="passing_year" value={passing_year} onChange={(e) => setPassingYear(e.target.value)}
                        className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                        required />
                </div>
                <div>
  ...
  {loading ? (
    <button type="button" disabled
        className="w-full h-11 mt-5 flex items-center justify-center text-white bg-blue-600 rounded-md cursor-not-allowed opacity-80 col-span-2">
        <ScaleLoader height={20} width={3} radius={2} margin={2} color="#ffffff" />
        <span className="ml-3">Creating Account...</span>
    </button>
  ) : (
    <button type="submit"
        className="cursor-pointer w-full h-11 px-4 mt-5 text-white bg-blue-600 rounded-md hover:bg-blue-700 transition duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 col-span-2">
        Create Account
    </button>
  )}
            </form>
